...
class TestAirForce(unittest.TestCase):
    ...
    def test_axial_force(self):
        ...
        result = af.axial_force(airspeed, air_density, body_area, axial_force_coefficient)
    ...
    def test_normal_force(self):
        ...
        result = af.normal_force(airspeed, air_density, body_area, normal_force_coefficient)
        norm = np.linalg.norm(result,ord=2)
        self.assertTrue(np.abs(float(norm)-840.0) < 1e-10)
        self.assertTrue(np.dot(result,np.cross(airspeed,np.array([1,0,0]))) < 1e-10)#対気速度と機体軸が張る平面上にある
