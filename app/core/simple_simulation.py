...
def air_force_body_frame(rocket_state:RocketState, config:Config) -> tuple[np.ndarray,np.ndarray]:
    """機体座標系での風圧による力を計算する
    ...
    """
    airspeed = quaternion_util.inertial_to_body(rocket_state.posture,rocket_state.velocity - config.wind(-rocket_state.position[2]))
    ...
    moment = af.air_force_moment(air_force, config.wind_center, config.propellant_CG_distance, config.nozzle_CG_distance, config.m_dot(t), rocket_state.rotation, config.roll_damping_coefficient, config.pitch_damping_coefficient, config.yaw_damping_coefficient, rocket_state.velocity, config.overall_length, 1.204, config.body_area)
    return air_force, moment

Gravitational_acceleration = np.array([0,0,9.8])

def simulate_launcher(first_state:RocketState, config:Config, first_time:float) -> list[tuple[float,RocketState]]:
    """ランチャー上でのシミュレーションを行う
    ...
    def derivative(t, state):
        ...
        virtual_force_body = quaternion_util.sum_vector_body_frame([air_force,thrust],[Gravitational_acceleration * config.mass(t)],state.posture)
        actual_force_inertial = quaternion_util.body_to_inertial(state.posture,np.array([max(0,virtual_force_body[0]),0,0]))
        return RocketState.derivative(state, actual_force_inertial / config.mass(t), np.zeros(3))

    def end_condition(t, state):
        return np.linalg.norm(state.position,ord=2) > config.launcher_length
    
    return ode_solver.runge_kutta4(derivative, first_state, first_time, config.dt, end_condition)

def simulate_flight(first_state:RocketState, config:Config, first_time:float,parachute_on:float) -> list[tuple[float,RocketState]]:
    """飛行中のシミュレーションを行う
    ...
    def derivative(t, state):
        ...
        thrust = np.array([config.thrust(t),0,0])
        force = quaternion_util.sum_vector_inertial_frame([air_force,thrust],[np.zeros(3)],state.posture)
        acceleration = force / config.mass(t) + Gravitational_acceleration
        angular_acceleration = equation_of_motion.angular_acceleration(moment, config.inertia_tensor, state.rotation)
...
def simulate(config:Config, parachute_on:float) -> list[list[tuple[float,RocketState]]]:
    """全体のシミュレーションを行う
    ...
    """
    first_posture = quaternion_util.from_euler_angle(config.first_elevation,config.first_azimuth,config.first_roll)
    ...
    result_flight = simulate_flight(result_launcher[-1][1], config, result_launcher[-1][0], parachute_on)
    if parachute_on:
        raise NotImplementedError("パラシュートを開いた時は未実装")
    else:
