...
def simulate(wind_direction,wind_speed):
    ...
    config.wind = wind.wind_velocity_power(reference_height,wind_speed,exponent,wind_direction)
    return simple_simulation.simulate(config,False)

def figure():
    with ProcessPoolExecutor() as e:
        r = list(e.map(simulate,range(0,360,45),[6]*8))
    x = [r[-1][-1][1].position[1] for r in r]
    y = [r[-1][-1][1].position[0] for r in r]
