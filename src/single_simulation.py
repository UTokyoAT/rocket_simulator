...
def figure(result):
    ...
    plt.plot(
        log.index / 1000 - 970.9, -log["x_gyro_raw"] * 0.003815 / 180 * np.pi, label="x"
    )
    plt.plot(
        log.index / 1000 - 970.9, -log["y_gyro_raw"] * 0.003815 / 180 * np.pi, label="y"
    )
    plt.legend()
    plt.grid()
    plt.title("rotation")
...
for r_ in r:
    ...
    plt.plot(
        t,
        [(x[1].rotation[1] ** 2 + x[1].rotation[2] ** 2) ** 0.5 for x in r_][: len(t)],
        label="simulation",
    )
plt.grid()
data = pd.read_csv("rotation.csv", index_col=0)
plt.plot(
    data.index / 1000 - 971.3, data["r"] * 0.003815 / 180 * np.pi, label="measurement"
)
plt.xlabel("Time [s]")
